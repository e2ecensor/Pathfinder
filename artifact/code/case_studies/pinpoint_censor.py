...
def dns_request(domain, server, ttl, timeout = 5):
    ...
    #     port = sock.getsockname()[1]  # Ensure port is assigned
    # except Exception as e:
    #     print(f"Error connecting socket: {e}")
    #     port = None  # Set a fallback

        addr = get_router_ip(icmp_sock, port if port else 0)
...
def http_request(domain, server, ttl, timeout = 5):
    ...
    try:
        ...
        # addr = get_router_ip(icmp_sock, port)
        addr = get_router_ip(icmp_sock, port if port else 0)
        # print(f"Router IP: {addr}")  # Debugging
    ...
    # http_result['device'] = addr if addr else 'Unknown'
